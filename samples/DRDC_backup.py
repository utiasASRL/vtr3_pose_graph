...
EXPERIMENTS = {
    ...
    "LTR vs. VirTR (clicked) vs. VirTR (driven)": {
        "Office": dict(
            ...
            method_1_runs=[13,15, 17,18, 20,21],  
...
def iter_repeats(graph, run_ids):
    ...
    for rid in run_ids:
        try:
            v_start = graph.get_vertex((rid, 5)) #ANTHONY
        except Exception:
            continue
        yield rid, v_start
    ...
    for m in methods:
        ...
        for i, repeat in enumerate(repeats):
            if m == 1:
                ...
                trial = 'VirTR (driven)'
            fname = f"{path_name}_{trial}_{i}"
            print(f"saving {fname}.pkl")
            with open(f"pkls/{fname}.pkl", "wb") as f:
                pkl.dump(repeat, f)
    ...
    for m in methods:
        ...
        if distances and len(distances) > 0:
            ...
            for _, _, cum_len_run, dists_run in repeats:
                ...
                mark_arrays.append(samples[1:]) # ANTHONY, for PTE@marks
            if mark_arrays:
                mark_samples_flat = np.concatenate(mark_arrays)
                if mark_samples_flat.size > 0:
                    print(mark_samples_flat)
