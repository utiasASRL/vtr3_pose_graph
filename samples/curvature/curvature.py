...
gs = GridSpec(
    1, 5,
    width_ratios=[0.5,0.5, 0.04, 0.15, 0.04],
    wspace=0.01,
)
...
# Plotting helper
# ============================
def plot_traj(ax, teach, repeat, zoom_xlim, zoom_ylim, y_flag=False):
    sc_teach = ax.scatter(
        teach['x'][1:], teach['y'][1:],
...
sc_teach, sc_err = plot_traj(ax_off, teach_office, repeat_office,
                              office_zoom_xlim, office_zoom_ylim, y_flag=True)
...
ax_off.set_title("Urban-R (Zoomed-In)", fontsize=14)
ax_urb.set_title("Structured-R (Zoomed-In)", fontsize=14)

# ============================
# Colorbars
# ============================
cb_curv = fig.colorbar(sc_teach, cax=cax_curv)
cb_curv.set_label(r"Teach Path Rate of Change of Curvature (m$^{-1}$)", fontsize=14)
cb_curv.ax.tick_params(labelsize=8)

cb_err = fig.colorbar(sc_err, cax=cax_err)
cb_err.set_label("Repeat Path Lateral Error (m)", fontsize=14)
