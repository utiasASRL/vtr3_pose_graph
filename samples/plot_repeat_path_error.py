...
if __name__ == '__main__':
    ...
    curvature = curvature_2d(np.hstack((xt, yt)))

    plt.figure(0)
    plt.scatter(xt[1:-1], yt[1:-1], label="Teach", c=curvature[2:]-curvature[:-2], cmap='rainbow', s=50) #'#D86900')
    plt.axis('equal')
    plt.grid()
    plt.colorbar(label=r"$\Delta$ Curvature")

    plt.figure(1)
    plt.scatter(xt, zt, label="Teach")

    # Process the repeat run: compute cumulative path length `p`
    vertex_start = test_graph.get_vertex((args.run, 5))
    ...
    z_bias = np.mean(zt) - np.mean(repeat_z)
    count = 0
    for v, e in TemporalIterator(vertex_start):    
        T_v_argo = v.T_v_w.r_ba_ina()
        T_v_argo[2] += z_bias

        repeat_z[count] += z_bias
        count +=1

        # dist.append(vtr_path.signed_distance_to_path(v.T_v_w.r_ba_ina(), path_matrix))
        dist.append(vtr_path.signed_distance_to_path(T_v_argo, path_matrix))
        path_len += np.linalg.norm(e.T.r_ba_ina())
        p.append(path_len)

    print(f'z bias: {z_bias}')
    ...
    if len(repeat_t) > 2:
        ...
        # c = np.array(c)
        # c = np.where(np.abs(c)>0.5, 0.5, c)
        # Overplot the repeat run on figure 0
        plt.figure(0)
        plt.scatter(repeat_x, repeat_y, cmap='viridis',label=f"Repeat (Max Error: {max_error:.3f}m)", c=c, s=10)
        plt.axis('equal')
        plt.xlabel('x (m)')
        plt.ylabel('y (m)')
        plt.colorbar(label="Lateral Error (m)")
        plt.savefig(f'plots/curvature/virtr_rural_pte.png')
        plt.legend()

        fname = f"virtr_office"
        ...
        with open(f"curvature/{fname}_t.pkl", "wb") as f:
            pkl.dump(teach_pkl, f)
        with open(f"curvature/{fname}_r.pkl", "wb") as f:
            pkl.dump(repeat_pkl, f)
